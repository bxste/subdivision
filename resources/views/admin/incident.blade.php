            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="py-6 px-8 border-b text-center">
                        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-3 rounded" onclick="toggleDetails({{ $loop->index }})">
                            View More
                        </button>
                    </td>
                    <td>
                        <form action="{{ route('admin.forms.updateStatus', $incidents_data) }}" method="POST" class="mt-4">
                            @csrf
                            @method('POST')
